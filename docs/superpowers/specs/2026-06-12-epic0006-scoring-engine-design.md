...
| D6 | **3-tier contributing-player tie-break:** final-over-provisional → earliest `updated_at` (best-effort recency hint) → lowest `player_id` (deterministic backstop). | Tiers 1 + 3 guarantee a deterministic, well-defined "BEST" winner. Tier 2 is heuristic ONLY: `trigger_set_updated_at` rewrites `updated_at` on every recompute, so it cannot reliably mean "first to hole out" and the badge is not stable across re-edits — determinism rests on tier 3 (`player_id`). Corrected after Lens review 2026-06-12 (earlier wording overstated stability). |
