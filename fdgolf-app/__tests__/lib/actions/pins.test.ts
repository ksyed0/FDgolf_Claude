...
const TEST_COURSE_ID = 'course-uuid-1'
...
describe('savePinAction', () => {
  beforeEach(() => {
    ...
    // mockEq chains: .eq('id', ...).eq('course_id', ...) resolves to { error: null }
    // Each eq() call returns a thenable that also exposes { eq: mockEq } for further chaining
    const successResult = Object.assign(Promise.resolve({ error: null }), { eq: mockEq })
    mockEq.mockReturnValue(successResult)
  ...
  it('returns error when admin check fails (RPC error)', async () => {
    ...
    const result = await savePinAction(TEST_COURSE_ID, { error: null }, makePinFormData())
  ...
  it('returns error when hole_id is missing', async () => {
    const result = await savePinAction(TEST_COURSE_ID, { error: null }, makePinFormData({ hole_id: '' }))
  ...
  it('returns error when lat is missing', async () => {
    ...
    fd.delete('lat')
    const result = await savePinAction(TEST_COURSE_ID, { error: null }, fd)
  ...
  it('returns error when lat is not a number', async () => {
    const result = await savePinAction(TEST_COURSE_ID, { error: null }, makePinFormData({ lat: 'abc' }))
    expect(result.error).toMatch(/valid numbers/i)
  })

  it('returns error when lng is not a number', async () => {
    const result = await savePinAction(TEST_COURSE_ID, { error: null }, makePinFormData({ lng: 'xyz' }))
    expect(result.error).toMatch(/valid numbers/i)
  })

  it('returns error when lat is out of range (>90)', async () => {
    const result = await savePinAction(TEST_COURSE_ID, { error: null }, makePinFormData({ lat: '91' }))
  ...
  it('returns error when lng is out of range (>180)', async () => {
    const result = await savePinAction(TEST_COURSE_ID, { error: null }, makePinFormData({ lng: '181' }))
  ...
  it('returns error when mode is invalid', async () => {
    const result = await savePinAction(TEST_COURSE_ID, { error: null }, makePinFormData({ mode: 'flag' }))
  ...
  it('returns error when DB update fails', async () => {
    const errorResult = Object.assign(Promise.resolve({ error: { message: 'update failed' } }), { eq: mockEq })
    mockEq.mockReturnValue(errorResult)
    const result = await savePinAction(TEST_COURSE_ID, { error: null }, makePinFormData())
  ...
  it('updates pin_lat/pin_lng when mode=pin', async () => {
    const result = await savePinAction(TEST_COURSE_ID, { error: null }, makePinFormData({ mode: 'pin', lat: '43.65', lng: '-79.38' }))
    expect(result.error).toBeNull()
    expect(mockUpdate).toHaveBeenCalledWith({ pin_lat: 43.65, pin_lng: -79.38 })
  })

  it('returns savedHoleNumber on success', async () => {
    const result = await savePinAction(TEST_COURSE_ID, { error: null }, makePinFormData({ hole_number: '5' }))
    expect(result.error).toBeNull()
    expect(result.savedHoleNumber).toBe(5)
  })

  it('calls revalidatePath for course and pins pages on success', async () => {
    await savePinAction(TEST_COURSE_ID, { error: null }, makePinFormData({ tournament_slug: 'my-tournament' }))
  ...
  it('does not call revalidatePath when tournament_slug is empty', async () => {
    await savePinAction(TEST_COURSE_ID, { error: null }, makePinFormData({ tournament_slug: '' }))
  ...
  it('updates tee_lat/tee_lng when mode=tee', async () => {
    const result = await savePinAction(TEST_COURSE_ID, { error: null }, makePinFormData({ mode: 'tee', lat: '43.66', lng: '-79.39' }))
    expect(result.error).toBeNull()
  ...
  // ─── Course ownership (BUG-0013) ────────────────────────────────────────────

  it('scopes update to courseId by calling eq with course_id', async () => {
    await savePinAction(TEST_COURSE_ID, { error: null }, makePinFormData())
    expect(mockEq).toHaveBeenCalledWith('id', 'hole-uuid-1')
    expect(mockEq).toHaveBeenCalledWith('course_id', TEST_COURSE_ID)
  })
})
