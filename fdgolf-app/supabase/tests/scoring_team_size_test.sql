...
SELECT is(
  (SELECT best_ball_score
     FROM team_hole_scores WHERE team_id = :'tm2' AND hole_number = 1),
  (SELECT MIN(gross_score) FROM hole_scores hs JOIN rounds r ON r.id = hs.round_id WHERE r.team_id = :'tm2' AND hs.hole_number = 1),
  'size 2: best_ball_score equals MIN gross across distinct member scores');
...
-- Size 5 — each member has a distinct gross so MIN selection is unambiguous
...
SELECT tests.add_shot(:'t5', :'a5', 1, 'sunk', 2, 1);          -- gross 2 final
SELECT tests.add_shot(:'t5', :'b5', 1, 'sunk', 1, 1);          -- gross 1 final (best)
SELECT tests.add_shot(:'t5', :'c5', 1, 'sunk', 3, 1);          -- gross 3 final
SELECT tests.add_shot(:'t5', :'d5', 1, 'sunk', 4, 1);          -- gross 4 final
SELECT tests.add_shot(:'t5', :'e5', 1, 'in_play', 1, 1);
SELECT tests.add_shot(:'t5', :'e5', 1, 'in_play', 1, 2);
SELECT tests.add_shot(:'t5', :'e5', 1, 'sunk',    3, 3);       -- gross 5 final
SELECT is(
  (SELECT best_ball_score || ':' || status::text
...
SELECT is(
  (SELECT best_ball_score
     FROM team_hole_scores WHERE team_id = :'tm5' AND hole_number = 1),
  (SELECT MIN(gross_score) FROM hole_scores hs JOIN rounds r ON r.id = hs.round_id WHERE r.team_id = :'tm5' AND hs.hole_number = 1),
  'size 5: best_ball_score equals MIN gross across distinct member scores');
