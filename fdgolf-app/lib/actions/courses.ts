...
function extractCourseFields(formData: FormData) {
  return {
    ...
    course_rating: formData.get('course_rating')
      ? parseFloat(formData.get('course_rating') as string)
      : null,
    slope_rating: formData.get('slope_rating')
      ? parseInt(formData.get('slope_rating') as string, 10)
      : null,
    tee_yardages: (() => {
...
export async function createCourseAction(
  ...
  const { name, holes_count, par_total, course_rating, slope_rating, tee_yardages } =
    extractCourseFields(formData)
